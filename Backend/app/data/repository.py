...
class TaskrRepository:
    ...
    def create_question(
        ...
        *,
        ...
        self.conn.execute(
            """
            INSERT INTO QUESTION (question_id, fk_node_state_id, hermes_task_id, hermes_run_id, prompt, options, status)
            VALUES (?, ?, ?, ?, ?, ?, 'open')
            """,
    ...
    def load_open_questions(self, run_id: str) -> list[dict[str, Any]]:
        ...
        rows = self.conn.execute(
            "SELECT q.* FROM QUESTION q JOIN NODE_STATE s ON s.node_state_id = q.fk_node_state_id WHERE s.fk_run_id = ? AND q.status = 'open' ORDER BY q.created_at, q.question_id",
            (run_id,),
        ).fetchall()
        return [self._row_to_dict(row, "QUESTION") for row in rows]
    ...
    def load_questions_for_node_state(self, node_state_id: str) -> list[dict[str, Any]]:
        ...
        rows = self.conn.execute(
            "SELECT * FROM QUESTION WHERE fk_node_state_id = ? ORDER BY created_at, question_id",
            (node_state_id,),
        ).fetchall()
        return [self._row_to_dict(row, "QUESTION") for row in rows]
    ...
    def load_open_questions_for_node_state(self, node_state_id: str) -> list[dict[str, Any]]:
        ...
        rows = self.conn.execute(
            "SELECT * FROM QUESTION WHERE fk_node_state_id = ? AND status = 'open' ORDER BY created_at, question_id",
            (node_state_id,),
        ).fetchall()
        return [self._row_to_dict(row, "QUESTION") for row in rows]
    ...
    def load_question(self, question_id: str) -> dict[str, Any] | None:
        ...
        row = self.conn.execute("SELECT * FROM QUESTION WHERE question_id = ?", (question_id,)).fetchone()
        return self._row_to_dict(row, "QUESTION") if row else None
    ...
    def save_question(self, question: dict[str, Any]) -> dict[str, Any]:
        ...
        self.conn.commit()
        return self.load_question(question["question_id"])
