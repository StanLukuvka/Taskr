...
def test_run_pauses_for_question_and_resumes():
    ...
    questions = repo.load_open_questions(run["run_id"])
    ...
    runner.answer_question(questions[0]["question_id"], "No, use organic sources only.")

    # Verify question is answered.
    q = repo.load_question(questions[0]["question_id"])
